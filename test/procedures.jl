...
@testset "MakeTreatCols" begin
    ...
    ret = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr), Dict(-1=>1),
    ...
    ret1 = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr), Dict(-1=>1),
        IdDict{ValidTimeType,Int}(11=>1))
    ...
    ret2 = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr), Dict(-1=>1),
        IdDict{ValidTimeType,Int}(11=>1))
    ...
    nt = merge(nt, (treatintterms=TermSet(term(:male)),))
    ret3 = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr), Dict(-1=>1),
        IdDict{ValidTimeType,Int}(11=>1))
    ...
    allntargs = NamedTuple[(tr=tr, pr=unspecifiedpr())]
    @test combinedargs(MakeTreatCols(), allntargs) ==
        (Dict(-1=>1), IdDict{ValidTimeType,Int}())

    df.wave = settime(Date.(hrs.wave), Year(1))
    df.wave_hosp = settime(Date.(hrs.wave_hosp), Year(1), start=Date(7))
    ret1 = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr),
    ...
    ret2 = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr),
    ...
    ret3 = maketreatcols(nt..., typeof(tr), tr.time, typeof(pr),
        Dict(-1=>1), IdDict{ValidTimeType,Int}(c=>1 for c in e))
